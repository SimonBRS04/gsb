...
<div class="users">
    ...
    <?= $this->Html->link(__d('cake_d_c/users', 'Changer le mot de passe'), ['plugin' => 'CakeDC/Users', 'controller' => 'Users', 'action' => 'changePassword']); ?>
    <div class="row">
        <div class="large-6 columns strings">
            <br/><h6 class="subheader"><?= __d('cake_d_c/users', 'Identifiant') ?></h6>
            <p><?= h($user->username) ?></p>
            <br/><h6 class="subheader"><?= __d('cake_d_c/users', 'Email') ?></h6>
            <p><?= h($user->email) ?></p>
            <?= $this->User->socialConnectLinkList($user->social_accounts) ?>
            <?php
            if (!empty($user->social_accounts)):
                ?>
                <h6 class="subheader"><?= __d('cake_d_c/users', 'Réseaux sociaux') ?></h6>
                <table cellpadding="0" cellspacing="0">
                    <thead>
                    <tr>
                        <th><?= __d('cake_d_c/users', 'Avatar'); ?></th>
                        <th><?= __d('cake_d_c/users', 'Provider'); ?></th>
                        <th><?= __d('cake_d_c/users', 'Lien'); ?></th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php
